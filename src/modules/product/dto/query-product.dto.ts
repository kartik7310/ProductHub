...
export class QueryProductDto {
    ...
    @ApiPropertyOptional({ description: 'Search by product name', example: 'headphones' })
    @IsString()
    @IsOptional()
    search?: string;
    ...
    @ApiPropertyOptional({ description: 'Number of items per page', example: 10, minimum: 1, default: 10 })
    @Type(() => Number)
    @IsNumber()
    @Min(1)
    @IsOptional()
